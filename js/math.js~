...
function testSolution(i, ifFinal)
{
	...
	//alert(ans.toString().substring(0,Math.min(inValue.length,ans.length)));
        if(inValue.toLowerCase()==ans.toString().toLowerCase())
        {
            input.style.backgroundColor = "#00ff00";
            numCorrect++;
        } else if(!ifFinal && (inValue.toLowerCase() == ans.toString().substring(0,Math.min(inValue.length,ans.toString().length)).toLowerCase()))
        {
